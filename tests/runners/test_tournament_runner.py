...
def test_headless_tournament_runner_executes_without_ui() -> None:
    ...
    assert result.matches[0].winner in (ControllerType.RANDOM_BOT, ControllerType.HEURISTIC_BOT, None)
    assert result.aggregates[ControllerType.RANDOM_BOT].games_played == 2
    assert result.aggregates[ControllerType.HEURISTIC_BOT].games_played == 2


def test_aggregation_correctness() -> None:
    matches = (
        MatchResult(
            lineup=(ControllerType.RANDOM_BOT,) * 4,
            seat_order=(ControllerType.RANDOM_BOT, ControllerType.HEURISTIC_BOT, ControllerType.RANDOM_BOT, ControllerType.HEURISTIC_BOT),
            seed=1,
            winner=ControllerType.RANDOM_BOT,
            final_vp_by_seat=(10, 8, 9, 7),
            turn_count=120,
            rank_by_seat=(1, 3, 2, 4),
            largest_army_holder=ControllerType.RANDOM_BOT,
            longest_road_holder=ControllerType.HEURISTIC_BOT,
        ),
    )

    aggregates = aggregate_results(matches)

    assert aggregates[ControllerType.RANDOM_BOT].wins == 2
    assert aggregates[ControllerType.RANDOM_BOT].games_played == 2
    assert aggregates[ControllerType.RANDOM_BOT].average_final_vp == 9.5
    assert aggregates[ControllerType.HEURISTIC_BOT].average_rank == 3.5
